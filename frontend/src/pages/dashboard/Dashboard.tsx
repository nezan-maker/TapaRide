...
function ClientDashboard() {
  ...
  const inTransitCount = recentParcels.filter(
    (p) => p.status === "IN_TRANSIT",
  ).length;
  const totalSpent = recentTickets.reduce(
    (sum, t) => sum + (t.journey?.price || 0),
    0,
  );
    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-ink-900 to-ink-700 p-6 text-white sm:p-8">
        ...
        <div className="relative">
          ...
          <Link
            to="/search"
            className="btn mt-5 bg-white text-ink-900 hover:bg-white/90"
          >
        ...
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <Stat
              icon="package"
              label="Parcels Sent"
              value={String(recentParcels.length)}
              hint={`${inTransitCount} in transit`}
            />
            <Stat
              icon="wallet"
              label="Total Spent"
              value={rwf(totalSpent)}
              hint="Lifetime"
            />
          </div>
          ...
          <Section title="Recent Trips" to="/dashboard/trips">
            {recentTickets.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="font-semibold text-ink-900">
                        {t.journey?.sourceStation?.name || "N/A"} →{" "}
                        {t.journey?.destinationStation?.name || "N/A"}
                      </div>
                      <div className="flex items-center gap-1.5 text-xs text-ink-400">
                        <Fa name="calendar" className="h-3 w-3" />{" "}
                        {new Date(
                          t.journey?.departureTime || t.createdAt,
                        ).toLocaleDateString()}
                      </div>
                    </div>
                    <span className="text-sm font-semibold text-ink-900">
                      {rwf(t.journey?.price || 0)}
                    </span>
                    <span
                      className={cn(
                        "chip",
                        statusStyles[t.status] || "bg-ink-100 text-ink-700",
                      )}
                    >
              ...
              <p className="py-4 text-center text-sm text-ink-400">
                No trips yet.{" "}
                <Link to="/search" className="font-semibold text-flame-600">
              ...
              <div className="divide-y divide-ink-100">
                {recentParcels.map((p) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="font-semibold text-ink-900">
                        {p.journey?.sourceStation?.name || "N/A"} →{" "}
                        {p.journey?.destinationStation?.name || "N/A"}
                      </div>
                    ...
                    <span
                      className={cn(
                        "chip",
                        statusStyles[p.status] || "bg-ink-100 text-ink-700",
                      )}
                    >
                      {p.status === "PENDING"
                        ? "Pending Pickup"
  ...
  return (
    <div className="card p-5 sm:p-6">
      <div className="mb-2 flex items-center justify-between">
        ...
        <Link
          to={to}
          className="inline-flex items-center gap-1 text-sm font-semibold text-flame-600 hover:gap-2"
        >
